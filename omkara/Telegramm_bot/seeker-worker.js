  ...
  try{
   ...
   if(item){
    ...
    let replies=await transaction(db,async c=>{
     ...
     if(!locked)return []; // Administrator deleted this queued update.
    ...
    while(replies.length){replies=await transaction(db,async c=>{
     const row=(await c.query('SELECT reply FROM telegram_updates WHERE update_id=$1 FOR UPDATE',[item.update_id])).rows[0];
     if(!row?.reply?.length)return [];
     await request('sendMessage',row.reply[0]);const remaining=row.reply.slice(1);
     await c.query('UPDATE telegram_updates SET reply=$2 WHERE update_id=$1',[item.update_id,JSON.stringify(remaining)]);return remaining;
    });}
 ...
 async function notify(){
  ...
  try{
   await transaction(db,async c=>{
    item=(await c.query("SELECT * FROM telegram_notifications WHERE state='pending' AND available_at<=now() ORDER BY id FOR UPDATE SKIP LOCKED LIMIT 1")).rows[0];
    if(!item)return;
    ...
    await c.query("UPDATE telegram_notifications SET state='sent',text='',attempts=attempts+1 WHERE id=$1",[item.id]);
   });
