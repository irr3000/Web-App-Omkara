import {poller} from './Telegramm_bot/telegram-poller.js';import {database} from './db.js';
...
const stopPoller=botToken&&process.env.TELEGRAM_TRANSPORT==='polling'?poller({db,botToken}).start():()=>{};const server=app.listen(Number(process.env.APP_PORT||3000),process.env.APP_IP||'127.0.0.1',()=>console.log('Omkara application started.'));
...
async function stop(){clearInterval(cleanup);stopPoller();stopTelegram();stopScheduler();server.close(async()=>{await db.end();process.exit(0);});setTimeout(()=>process.exit(1),10000).unref();}
