import {register,submitReceipt,reviewRegistration,cancelRegistration,registrations,saveProfile} from './seeker-service.js';
...
import {installSchedulerRoutes} from './Telegramm_bot/admin-scheduler.js';
import {installAdminDeletion} from './admin-delete.js';
...
export function createApp({db,mail,origin,secure=true,botUsername='',telegramSecret='',ownerEmail='',telegramChannels=[],telegramReady=false}){
 const app=express();
 ...
 app.patch('/api/profile',member,async(req,res)=>res.json({user:await saveProfile(db,req.user.id,req.body)}));
 ...
 app.get('/api/registrations',member,async(req,res)=>{
  res.json(await registrations(db,req.user.id));
 });
 app.post('/api/registrations',member,async(req,res)=>{
  const eventId=uuid(req.body.event_id);const result=await transaction(db,async c=>{
   return register(c,req.user.id,eventId);
  });res.status(201).json(result);
 });
 app.post('/api/registrations/:id/cancel',member,async(req,res)=>{
  await transaction(db,c=>cancelRegistration(c,req.user.id,uuid(req.params.id)));res.json({ok:true});
 });
 ...
 app.post('/api/registrations/:id/receipt',member,async(req,res,next)=>{await limit(req,'receipt',20,900,req.user.id);next();},upload,async(req,res)=>{
  check(req.file,'Выберите файл чека.');await transaction(db,c=>submitReceipt(c,req.user.id,uuid(req.params.id),req.file.buffer));res.json({ok:true});
 });
 ...
 app.get('/api/admin/people',admin,async(req,res)=>{const offset=Math.max(0,Math.min(1000000,Number(req.query.offset)||0));res.json((await db.query(`SELECT ${ownFields},created_at FROM people WHERE merged_into IS NULL ORDER BY created_at DESC LIMIT 100 OFFSET $1`,[offset])).rows);});
 ...
 app.post('/api/admin/registrations/:id/review',admin,async(req,res)=>{
  await transaction(db,c=>reviewRegistration(c,req.user.id,uuid(req.params.id),req.body));res.json({ok:true});
 });
 ...
 installSchedulerRoutes(app,{db,admin,limit,uuid,channels:telegramChannels,telegramReady});
 installAdminDeletion(app,{db,admin,uuid});
