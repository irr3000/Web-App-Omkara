(() => {
 ...
 const date=v=>v?new Intl.DateTimeFormat('ru-RU',{dateStyle:'long',timeStyle:'short',timeZone:'Asia/Yekaterinburg'}).format(new Date(v)):'Дата и время по согласованию';
 ...
 root.addEventListener('click',event=>{const button=event.target.closest('[data-action]');if(!button)return;const {action,id}=button.dataset;if(action==='scroll'){const target=document.getElementById(id);target?.scrollIntoView({block:'start'});if(target?.hasAttribute('tabindex'))target.focus({preventScroll:true});return;}if(action==='go'){offset=0;go(id);return;}mutate(async()=>{
  if(action==='delete-person'||action==='delete-event'){
   const text=action==='delete-person'?'Удалить анкету и все связанные заявки, оплаты, чеки и привязку Telegram? Восстановление через кабинет невозможно.':'Удалить ретрит или встречу, включая завершённые участия, оплаты и чеки? Общие анкеты ищущих сохранятся. Восстановление через кабинет невозможно.';
   if(!confirm(text))return;
   await api('/admin/'+(action==='delete-person'?'people/':'events/')+id,'DELETE');
   events=await api('/events');await render();message('Удалено из базы вместе со связанными данными.');
  }
 ...
 root.addEventListener('submit',event=>{event.preventDefault();const form=event.target,values=Object.fromEntries(new FormData(form)),decision=event.submitter?.value;mutate(async()=>{
  ...
  }else if(form.id==='event'){const id=form.dataset.id;await api('/admin/events'+(id?'/'+id:''),id?'PATCH':'POST',{...values,price_kop:Math.round(Number(values.price)*100),capacity:values.capacity||null,starts_at:values.starts_at?new Date(values.starts_at+'+05:00').toISOString():null,ends_at:values.ends_at?new Date(values.ends_at+'+05:00').toISOString():null,published:values.published==='on',registration_open:values.registration_open==='on'});events=await api('/events');go('admin/events');}
 });});
