...
const app=createApp({db,mail:mailer(),origin,secure,botUsername:botToken?process.env.TELEGRAM_BOT_USERNAME:'',telegramSecret,ownerEmail:process.env.OWNER_EMAIL||''});
...
async function stop(){clearInterval(cleanup);stopTelegram();server.close(async()=>{await db.end();process.exit(0);});setTimeout(()=>process.exit(1),10000).unref();}
