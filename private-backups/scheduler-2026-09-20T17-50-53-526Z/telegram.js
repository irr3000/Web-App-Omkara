...
export async function telegramReply(db,update,origin){
 ...
 return {chat_id:chatId,text:person?'Ретриты, встречи и отправка чеков доступны в личном кабинете. Команда /requests показывает ваши записи.':'Добро пожаловать. Откройте сайт для записи на ретриты и встречи. Чтобы связать Telegram с кабинетом, нажмите «Подключить Telegram» в своей анкете.',reply_markup:{inline_keyboard:[[{text:'Открыть приложение',url:origin}]]}};
}
