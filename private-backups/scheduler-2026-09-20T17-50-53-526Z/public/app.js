 ...
 async function render(){
  ...
  try{
   if(path.startsWith('verify=')){
    ...
    const nav=`<div class="actions">${btn('Мероприятия','go','admin/events',true)}${btn('Записи и чеки','go','admin/registrations',true)}${btn('Ищущие','go','admin/people',true)}</div>`;
    if(path==='admin/events'){
     ...
     root.innerHTML=`<p class="eyebrow">Администратор</p><h1>Ищущие</h1>${nav}${rows.map(p=>`<article class="card body application"><h2>${esc(p.first_name)} ${esc(p.last_name)}</h2><p>${esc(p.email)}<br>${esc(p.phone)}</p><p class="meta">${esc(date(p.created_at))}</p></article>`).join('')}${pager(rows.length)}`;
