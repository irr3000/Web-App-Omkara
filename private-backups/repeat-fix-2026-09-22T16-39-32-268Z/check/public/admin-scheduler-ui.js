...
export async function mountScheduler({root,api,esc,nav,isCurrent}){
 let data=await api('/admin/scheduled-posts');if(!isCurrent())return;
 ...
 function addRow(p={}){
  ...
  function drawChats(){const list=tr.querySelector('.selected-chats');list.replaceChildren();for(const id of selected){const chip=document.createElement('button');chip.type='button';chip.className='chat-chip';chip.textContent=chatName(id)+' ×';chip.setAttribute('aria-label','Убрать чат '+chatName(id));chip.disabled=p.id&&!['pending','sent','expired'].includes(p.status);chip.onclick=()=>{selected.delete(id);for(const box of options.querySelectorAll('input'))box.checked=selected.has(box.value);drawChats();};list.append(chip);}tr.querySelector('.scheduler-chats summary').textContent=selected.size?'Выбрано чатов: '+selected.size:'Выбрать чаты';}
  drawChats();
  ...
  function sync(){
   const editable=!p.id||['pending','sent','expired'].includes(p.status);
   ...
   tr.querySelector('.save').textContent=p.status==='pending'?(p.enabled?'Сохранить изменения':'Возобновить'):'Запланировать';
  ...
  async function save(){
   ...
   try{
    ...
    const values={expected_updated_at:p.updated_at,text,publish_at,repeat:tr.querySelector('.repeat').value,channels:[...selected],expires_at:expiry?yekaterinburgUTC(expiry):null,buttons:[...bs.children].map(b=>({text:b.querySelector('[type=text]').value,url:b.querySelector('[type=url]').value})),enabled:true};
    ...
    if(file){const form=new FormData();form.set('photo',file);await api(`/admin/scheduled-posts/${p.id}/photo`,'POST',form);tr.querySelector('[type=file]').value='';photoLink();p={...p,...await api('/admin/scheduled-posts/'+p.id,'PATCH',{...values,expected_updated_at:p.updated_at})};}
    dirty=false;
    if(!p.id||!p.enabled)throw Error('Публикация не включена в расписание. Повторите сохранение.');
    records.delete(tr);records.set(p.id,record);tr.querySelector('.delivery-status').textContent='';
  ...
  tr.querySelector('.remove').onclick=async()=>{if(p.id&&!confirm('Удалить строку и прекратить будущие отправки? Уже назначенное удаление сообщений из Telegram будет выполнено.'))return;try{if(p.id)await api(`/admin/scheduled-posts/${p.id}`,'DELETE');tr.remove();records.delete(p.id||tr);status.textContent='Строка удалена.';}catch(e){status.textContent=e.message;}};
  const record={tr,snapshot:()=>p,deliveries:JSON.stringify(current),hasEdits:()=>dirty||saving||tr.contains(document.activeElement),dirty:()=>dirty||saving,changed:()=>{tr.querySelector('.post-status').textContent='Расписание обновлено на сервере; есть несохранённые правки';}};
  tr.addEventListener('input',()=>dirty=true);tr.addEventListener('change',()=>dirty=true);
  tr.addEventListener('click',e=>{if(e.target.closest('.add-button,.scheduler-button button,.chat-chip'))dirty=true;});
  sync();tr.querySelectorAll('.recovery input').forEach(e=>e.disabled=false);tbody.append(tr);
  if(p.id)records.set(p.id,record);else records.set(tr,record);
  return tr;
 }
 ...
 async function refresh(){
  if(refreshing||!isCurrent())return;refreshing=true;
  try{
   const latest=await api('/admin/scheduled-posts');if(!isCurrent())return;
   data=latest;
   for(const p of data.posts){const old=records.get(p.id),was=old?.snapshot();
    if(old?.hasEdits()){if(was&&was.updated_at!==p.updated_at)old.changed();continue;}
    const changed=!was||JSON.stringify(was)!==JSON.stringify(p)||old?.deliveries!==JSON.stringify(data.deliveries.filter(d=>d.post_id===p.id));
    if(old&&!changed)continue;
    const tr=addRow(p);if(old)old.tr.replaceWith(tr);
   }
   for(const [id,r] of records)if(typeof id==='string'&&!data.posts.some(p=>p.id===id)&&!r.hasEdits()){r.tr.remove();records.delete(id);}
  }catch(e){if(isCurrent())status.textContent='Не удалось обновить статусы: '+e.message;}
  finally{refreshing=false;}
 }
 const timer=setInterval(()=>{if(!isCurrent()||!root.contains(tbody)){clearInterval(timer);return;}void refresh();},5000);
 root.querySelector('.scheduler-refresh').onclick=()=>{
  if([...records.values()].some(r=>r.dirty())&&!confirm('Обновить строки с сервера и отменить несохранённые правки?'))return;
  clearInterval(timer);mountScheduler({root,api,esc,nav,isCurrent}).catch(e=>{status.textContent=e.message;});
 };
}
