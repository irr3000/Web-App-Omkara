...
import {recordSchedulerChat} from './Telegramm_bot/scheduler-chats.js';
...
export function installTelegramWebhook(app,{db,secret}){
 app.post('/telegram/webhook',async(req,res)=>{
  ...
  await transaction(db,async c=>{await c.query('INSERT INTO telegram_updates(update_id,payload) VALUES($1,$2) ON CONFLICT DO NOTHING',[req.body.update_id,JSON.stringify(req.body)]);await recordSchedulerChat(c,req.body);});
  res.json({ok:true});
 });
}
