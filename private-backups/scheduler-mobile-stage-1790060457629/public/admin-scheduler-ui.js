export async function mountScheduler({root,api,esc,nav,isCurrent}){
 ...
 function addRow(p={}){
  const tr=document.createElement('tr'),iso=p.publish_at?new Date(p.publish_at).toISOString():new Date(Date.now()+3600000).toISOString();
  tr.innerHTML=`<td><textarea aria-label="Текст публикации" maxlength="4096">${esc(p.text||'')}</textarea></td><td>${input('date',iso.slice(0,10),'Дата')}</td><td>${input('time',iso.slice(11,16),'Время UTC')}</td><td><select class="repeat" aria-label="Повтор">${[['once','Однократно'],['daily','Ежедневно'],['weekly','Еженедельно']].map(([v,t])=>`<option value="${v}" ${p.repeat===v?'selected':''}>${t}</option>`).join('')}</select></td><td><details class="scheduler-chats"><summary>Выбрать чаты</summary><div class="chat-options"></div></details><div class="selected-chats"></div></td><td>${input('datetime-local',p.expires_at?new Date(p.expires_at).toISOString().slice(0,16):'','Удаление из Telegram UTC')}</td><td><div class="buttons"></div><button type="button" class="add-button">Добавить кнопку</button></td><td><input type="file" accept="image/png,image/jpeg" aria-label="Фото"><span class="photo-status">${p.has_photo?'Фото загружено':''}</span></td><td><strong class="post-status"></strong><p class="delivery-status"></p><button type="button" class="save">Запланировать</button><button type="button" class="send">Отправить сейчас</button><button type="button" class="pause">Приостановить</button><button type="button" class="remove">Удалить строку</button><div class="recovery"></div></td>`;
  ...
  function sync(){
   ...
   tr.querySelector('.save').hidden=!editable;tr.querySelector('.send').hidden=!editable;tr.querySelector('.send').disabled=!data.telegram_ready;
   tr.querySelector('.pause').hidden=!(p.id&&p.status==='pending'&&p.enabled);
   tr.querySelector('.save').textContent=p.id&&!p.enabled?'Запланировать / возобновить':'Запланировать';
  ...
  async function save(sendNow=false){
   ...
   try{
    ...
    const publish_at=new Date(tr.querySelector('[type=date]').value+'T'+tr.querySelector('[type=time]').value+':00Z').toISOString(),expiry=tr.querySelector('[type=datetime-local]').value;
    const values={text,publish_at,repeat:tr.querySelector('.repeat').value,channels:[...selected],expires_at:expiry?new Date(expiry+'Z').toISOString():null,buttons:[...bs.children].map(b=>({text:b.querySelector('[type=text]').value,url:b.querySelector('[type=url]').value})),enabled:true};
    ...
    const saved=await api('/admin/scheduled-posts'+(p.id?'/'+p.id:''),p.id?'PATCH':'POST',{...values,enabled:file||sendNow?false:true});p={...p,...saved};
    if(file){const form=new FormData();form.set('photo',file);await api(`/admin/scheduled-posts/${p.id}/photo`,'POST',form);tr.querySelector('[type=file]').value='';photoLink();if(!sendNow)p={...p,...await api('/admin/scheduled-posts/'+p.id,'PATCH',values)};}
    if(sendNow){await api(`/admin/scheduled-posts/${p.id}/send`,'POST');p.status='sending';p.enabled=true;}
    status.textContent=sendNow?'Публикация поставлена в очередь. Обновите статусы через несколько секунд.':'Публикация запланирована.';
   }catch(e){status.textContent=e.message;}
   finally{tr.querySelectorAll('button').forEach(b=>b.disabled=false);sync();}
  }
  tr.querySelector('.save').onclick=()=>save();tr.querySelector('.send').onclick=()=>save(true);
