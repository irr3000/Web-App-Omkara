export function telegramAPI(botToken, fetcher=fetch){
 return async(method,payload,photo)=>{
  ...
  if(!result.ok)throw Object.assign(Error(`Telegram: ${result.error_code||response.status}`),{telegramDescription:String(result.description||`Telegram: ${result.error_code||response.status}`).split(botToken).join('[redacted]').slice(0,500),retryAfter:result.parameters?.retry_after||300,permanent:[400,401,403].includes(result.error_code)});
