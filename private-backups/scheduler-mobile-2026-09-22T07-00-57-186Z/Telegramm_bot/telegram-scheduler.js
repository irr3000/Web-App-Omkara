 ...
 async function tick(){
  ...
  try{
   ...
   await transaction(db,async c=>{
    ...
    for(const p of posts){
     const ds=(await c.query('SELECT state FROM post_deliveries WHERE post_id=$1 AND occurrence=$2',[p.id,p.publish_at])).rows;
     ...
     await c.query('UPDATE scheduled_posts SET publish_at=COALESCE($2,publish_at),enabled=$3,status=$4 WHERE id=$1',[p.id,again?next:null,again,again?'pending':'sent']);
   ...
   await transaction(db,async c=>{
    const d=(await c.query("SELECT * FROM post_deliveries WHERE state='sent' AND NOT deleted AND delete_at<=now() AND available_at<=now() ORDER BY id FOR UPDATE SKIP LOCKED LIMIT 1")).rows[0];if(!d)return;
    try{await send('deleteMessage',{chat_id:d.chat_id,message_id:d.message_id});await c.query('UPDATE post_deliveries SET deleted=true,error=NULL WHERE id=$1',[d.id]);}
    catch{await c.query("UPDATE post_deliveries SET error='Не удалось удалить сообщение: проверьте права и ограничения Telegram.',available_at=now()+interval '1 hour' WHERE id=$1",[d.id]);}
   });
