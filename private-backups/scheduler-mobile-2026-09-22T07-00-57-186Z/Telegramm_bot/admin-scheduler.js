...
export function installSchedulerRoutes(app,options){
 ...
 app.get('/api/admin/scheduled-posts',admin,async(_req,res)=>res.json({channels,telegram_ready:telegramReady,posts:(await db.query('SELECT p.*,EXISTS(SELECT 1 FROM post_photos f WHERE f.post_id=p.id) has_photo FROM scheduled_posts p ORDER BY created_at DESC LIMIT 200')).rows,deliveries:(await db.query('SELECT id,post_id,chat_id,state,error,delete_at,deleted FROM post_deliveries ORDER BY id DESC LIMIT 500')).rows}));
 app.post('/api/admin/scheduled-posts',admin,async(req,res)=>{const values=postValues(req.body,channels);const result=await db.query('INSERT INTO scheduled_posts(id,text,publish_at,repeat,channels,buttons,expires_at,enabled,created_by) VALUES($1,$2,$3,$4,$5,$6,$7,$8,$9) RETURNING *',[randomUUID(),...values,req.user.id]);res.status(201).json(result.rows[0]);});
 app.patch('/api/admin/scheduled-posts/:id',admin,async(req,res)=>{
  const id=uuid(req.params.id);const updated=await transaction(db,async c=>{
   ...
   check(p.status==='pending','Отправка завершена или требует проверки; создайте новую строку.',409);
   ...
   const values=postValues({...p,publish_at:new Date(p.publish_at).toISOString(),...req.body},channels);
   ...
   return (await c.query('UPDATE scheduled_posts SET text=$2,publish_at=$3,repeat=$4,channels=$5,buttons=$6,expires_at=$7,enabled=$8,updated_at=now() WHERE id=$1 RETURNING *',[id,...values])).rows[0];
  });res.json(updated);
 });
